...
const Render = {
    async render(path, params, cfg) {
        ...
        return e.runtime.render('imgS-plugin', path, params, {
            retType: cfg.retMsgId ? 'msgId' : 'default',
            beforeRender({ data }) {
                let pluginName = ''
                if (data.pluginName !== false) {
                    pluginName = ` & ${data.pluginName || 'imgS-plugin'}`
                    if (data.pluginVersion !== false) {
                        pluginName += `<span class="version">${currentVersion}`
                    }
                }
                let resPath = data.pluResPath
                const layoutPath = process.cwd() + '/plugins/imgS-plugin/resources/common/layout/'
                return {
                    ...data,
                    _res_path: resPath,
                    _imgs_path: resPath,
